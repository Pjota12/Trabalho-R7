...
int hash_camp_ano(int ano){
  return ano % TAM_HASH_CAMP_ANO;
}

void THcamp_ano_inicializa(char *tabHash, char *dados){
    FILE *fh = fopen(tabHash,"wb"), *fd = fopen(dados,"wb");
    if((!fh) || (!fd)) exit(1);
    // Incializa hash
    int end = -1;
    for(int i = 0; i < TAM_HASH_CAMP_ANO; i++) fwrite(&end,sizeof(int),1,fh);
    fclose(fh);
    fclose(fd);
}
  
char *THcamp_ano_busca_sobrenome(char *tabHashNome, char *dadosNome, char *sobrenome) {
...
void THcamp_ano_insere(char *arqHash, char *arqDados, char *id, int ano) {
    ...
    while(pos != -1) {
        fseek(fd, pos, SEEK_SET);
        fread(&aux, sizeof(THcamp_ano), 1, fd);
    ...
    if(ppl == -1) {
        ...
        pos = ftell(fd);
        fwrite(&elem, sizeof(THcamp_ano), 1, fd);
        if(ant == -1) {
            fseek(fh, hash * sizeof(int), SEEK_SET);
            ...
            fwrite(&aux, sizeof(THcamp_ano), 1, fd);
        }
        fclose(fh);
        fclose(fd);
        return;
...
void THcamp_ano_construcao(char *arqChampions, char *arqHash, char *arqDados) {
    ...
    THcamp_ano_inicializa(arqHash,arqDados);
    ...
    while (fgets(linha, sizeof(linha), fp)) {
        ...
        while (token && coluna < 15) { 
            if (coluna >= 0) {
                ...
                if (strcmp(token, "-") != 0) {
                    char *id = THcamp_ano_busca_sobrenome("hash_nome.bin", "dados_nome.bin", token);
                    if (id) {
...
char *THcamp_ano_busca(char *arqHash, char *arqDados, char *id, int ano) {
    ...
    int pos, hash = hash_camp_ano(ano);
...
void THcamp_ano_remove(char *arqHash, char *arqDados, char *id) {
    ...
    while(r == 1) {
        ...
        while(pos != -1) {
            fseek(fd, pos, SEEK_SET);
            fread(&player,sizeof(THcamp_ano),1,fd);

            if(!strcmp(player.id, id)) {
                if(player.status) {
                    player.status = 0;
                    fseek(fd, pos, SEEK_SET);
                    fwrite(&player,sizeof(THcamp_ano),1,fd);
                }
